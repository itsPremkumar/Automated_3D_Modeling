...
def get_latest_version_dir(outputs_base: str) -> Optional[str]:
    """
    Scans the outputs directory to find the most recently created version folder.

    Args:
        outputs_base (str): The absolute path to the base outputs directory.

    Returns:
        Optional[str]: The absolute path to the latest version directory (e.g., 'outputs/v011'), 
                       or None if no directories exist.
    """
...
def run_orchestrator() -> None:
    """
    The main autonomous loop for the AI Orchestrator.
    
    This function continuously loops to:
    1. Read the current configuration from `config.json`.
    2. Modify physical CAD parameters (e.g., reinforcing wall thickness) if necessary.
    3. Execute the Universal CAD Pipeline (`master_pipeline.py`) via subprocess.
    4. Parse the output `fitness.json` scorecard generated by the pipeline.
    5. Decide whether to successfully exit (design passed constraints) or iterate again.
    """
    ...
    for iteration in range(max_iterations):
        ...
        if not latest_dir:
            logging.error("No output directory found. Pipeline crashed.")
            break
        ...
        if os.path.exists(fitness_file):
            with open(fitness_file, "r") as f:
                fitness: Dict[str, Any] = json.load(f)
                
            stress = fitness.get("stress_mpa", 0)
            safe = fitness.get("structurally_safe", False)
            mass = fitness.get("mass_g", 0)
