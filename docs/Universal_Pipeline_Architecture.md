...
```mermaid
graph TD
    A[AI Orchestrator] -->|Injects Config| B(Pipeline Engine)
    C[CAD Plugins] -->|Geometry| B
    D[External .step Files] -->|Boolean Edits| C
    
    B --> E{Validation & Repair}
    E -->|Topology Check| F[Trimesh Healer]
    
    F --> G{Fusion 360 MCP}
    G --> H[Collision Detection]
    G --> I[Exploded View Renders]
    G --> J[Native F3D Export]
    
    B --> K[Manifest & BOM]
    B --> L[Dynamic URDF XML]
```
...
When the Engine (`master_pipeline.py`) receives a generated or modified assembly, it automatically executes the following industrial workflows:
